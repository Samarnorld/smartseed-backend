...
def recommend_seed(soil, risk, suit):
    ...
    if risk.get("drought") is not None and risk["drought"] > 30:
        return ["DH04", "KDV4"]
...
def recommend_fertilizer(soil):
    ...
    if soil.get("phosphorus") is not None and soil["phosphorus"] < 15:
        rec.append("DAP at planting")

    if soil.get("nitrogen") is not None and soil["nitrogen"] < 0.2:
